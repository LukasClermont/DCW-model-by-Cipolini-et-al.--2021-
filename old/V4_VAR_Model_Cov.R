...
#Standard ARMA model - DCW
{
  ...
  #Plot Prediction
  df.pred <- cbind(df.date ,realoptweights$MSFT, omega$MSFT, optweights.RW$MSFT, optimalweights.uncond$MSFT)
  colnames(df.pred) <- c("date", "Realized optimal portfolio weights", "DCW", "RW", "Naive")
  df.pred <- df.pred %>% transform( `Realized optimal portfolio weights` = as.numeric(`Realized optimal portfolio weights`),
                                    DCW = as.numeric(DCW),
                                    RW = as.numeric(RW),
  ...
  #Restricted
  Modeltype_ARMA <- c(1, 0, 1)
  epsilon.rest <- realoptweights.positive[1]
  omega.rest <- realoptweights.positive[1]
  data_aic.rest <- data.frame()
  for(i in 2:ncol(realoptweights.positive)){
    y <- ts(realoptweights.positive[i])
    arma11 <- arima(y, order = Modeltype_ARMA, method ="ML")
    epsilon_t <- as.data.frame(residuals(arma11))
    colnames(epsilon_t) <- paste0("V",i-1)
    epsilon.rest <- cbind(epsilon, epsilon_t)
    omega_t <- as.data.frame(y) - epsilon_t
    colnames(omega_t) <- i-1
    omega.rest <- cbind(omega, omega_t)
  }
  colnames(omega.rest) <- c("date", lable[1,])
  colnames(epsilon.rest) <- c("date", lable[1,])
  
  df.pred.positiv <- cbind(df.date ,realoptweights.positive$MSFT, omega.rest$MSFT)
