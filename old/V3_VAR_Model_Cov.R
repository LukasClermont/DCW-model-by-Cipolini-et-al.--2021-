...
#No-short-Portfolio
{
  ...
  D_matrix <- 2 * list.realCov.Date$R[,,1]
  d_vector <- rep(0, 30)
  A_matrix <- cbind(rep(1, 30), diag(30))
  b_vector <- c(1, rep(0, 30))
  
  weights <- quad_prog <- solve.QP(Dmat = D_matrix, dvec = d_vector, Amat = A_matrix, bvec = b_vector, meq = 1)
  weights$solution
  get.portfolio.variance(t(as.matrix(weights$solution)), list.realCov.Date$R[,,1])
  get.portfolio.mean.return(t(as.matrix(weights$solution)), as.matrix(return[1,]))
  
  weigths.realiesed <- realoptweights[-1]
  get.portfolio.variance(as.matrix(weigths.realiesed[1,]), as.matrix(list.realCov.Date$R[,,1]))
  get.portfolio.mean.return(as.matrix(weigths.realiesed[1,]), as.matrix(return[1,]))
}
df.realoptweights
